...
int main(void){
	...
	int n;
	while(1){
		printf("请输入您要选择的功能号（0-创建文件/1-创建目录/2-输入囧对路径找相应的inode/3-退出）:");
		scanf("%d",&n);
		if(n == 0){
			while(1){
	...
	printf("root.ext2_inode.i_blocks: %d\n",root.ext2_inode.i_blocks);
	printf("inode    \trec_len  \tname_len \tfile_type\tname\n");
	while(i <= root.ext2_inode.i_blocks){
		get_block_data(root.ext2_inode.i_block[i],buf);
	...
	while(i <= root.ext2_inode.i_blocks){
		printf("root.ext2_inode.i_block[%d]: %d\n",i,root.ext2_inode.i_block[i]);
		i++;
	}
